...
import { RegistrationDetails, CountryMatrixRow } from "../types";

interface PortfolioStatusSelectorProps {
  country: string;
  committee: "copuos" | "disec" | "aippm" | "unsc";
  currentStatus: "Available" | "Assigned" | "Reserved";
  onUpdate: (country: string, committee: "copuos" | "disec" | "aippm" | "unsc", newStatus: "Available" | "Assigned" | "Reserved") => void;
}
...
export default function AdminDashboard() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<"registrations" | "portfolios">("registrations");
  ...
  const updatePortfolioStatus = async (
    country: string,
    committee: "copuos" | "disec" | "aippm" | "unsc",
    newStatus: "Available" | "Assigned" | "Reserved"
  ) => {
    try {
  ...
  const getPortfolioStatus = (
    countryName: string,
    committee: "copuos" | "disec" | "aippm" | "unsc",
    defaultStatus: "Available" | "Assigned" | "Reserved"
  ) => {
  ...
  return (
    <div className="bg-[#020617] text-slate-100 min-h-screen py-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden" style={{ backgroundImage: "radial-gradient(circle at 50% -20%, #1e293b 0%, #020617 80%)" }}>
      ...
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-full border border-blue-500/30 bg-slate-900/95 px-5 py-3 shadow-[0_0_20px_rgba(37,99,235,0.3)] backdrop-blur-md animate-fade-in">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
                        ...
                        >
                          ...
                          <td className="p-4">
                            <div className="flex flex-col text-left">
                              <span className="font-sans text-sm font-bold text-white leading-tight">{item.name}</span>
        ...
        {selectedReg && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-2xl w-full p-6 relative overflow-hidden shadow-2xl max-h-[85vh] overflow-y-auto">
              ...
              <div className="text-left border-b border-slate-800 pb-4 mb-6">
                ...
                <h3 className="font-sans text-xl font-bold text-white mt-1">{selectedReg.name}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                ...
                {selectedReg.regType === "double" && (
                  <>
                    <div>
                      <span className="font-mono text-[9px] text-slate-500 uppercase tracking-wider block">Partner Name</span>
                      <p className="font-sans text-sm text-slate-200 font-semibold mt-0.5">{selectedReg.partnerName || "N/A"}</p>
                    </div>
                    <div>
                      <span className="font-mono text-[9px] text-slate-500 uppercase tracking-wider block">Partner Email</span>
                      <p className="font-sans text-sm text-slate-200 font-semibold mt-0.5">{selectedReg.partnerEmail || "N/A"}</p>
                    </div>
                  </>
                )}
        ...
        {confirmDeleteId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-sm w-full p-6 text-center relative overflow-hidden shadow-2xl">
